...
function Reviews() {
    ...
    const loadReviews = async () => {
        try {
            setLoading(true);
            setError("");
            const res = await adminClient.get("/content/reviews");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                await adminClient.put(`/content/reviews/${editingId}/status`, {
                    status: payload.status
                });
            } else {
                // Create new review
                await adminClient.post("/content/reviews", payload);
            }
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this review?")) {
            try {
                await adminClient.delete(`/content/reviews/${id}`);
                await loadReviews();
            } catch (err) {
