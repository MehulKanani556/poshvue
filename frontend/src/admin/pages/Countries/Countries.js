...
function Countries() {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        setLoading(true);
        setError("");
        const res = await adminClient.get("/country", { params: { page: 1, limit: 50 } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError("");
      if (editingId) {
        const res = await adminClient.put(`/country/${editingId}`, formData);
        setCountries((prev) => prev.map((c) => (c._id === editingId ? res.data.item : c)));
      } else {
        const res = await adminClient.post("/country", formData);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      await adminClient.delete(`/country/${deletingId}`);
