...
async function getToken() {
  ...
  try {
    ...
    const msg = typeof data === 'string'
      ? data
      : (data?.message || data?.error || data?.msg || data?.errors || (data && JSON.stringify(data)));

    const fullMsg = msg || err.message;
    console.error('[Shiprocket] Token request failed:', status, fullMsg);
    if (data) console.error('[Shiprocket] Response body:', typeof data === 'string' ? data : JSON.stringify(data));
    ...
    if (status === 401) {
      throw new Error('Shiprocket 401: Invalid email or password. ' + (fullMsg ? String(fullMsg) : ''));
    }
...
exports.createShipmentForOrder = async (order) => {
  ...
  if (!token) {
    throw new Error('Shiprocket: No token. Add SHIPROCKET_EMAIL and SHIPROCKET_PASSWORD (API user) in .env.');
  }
  ...
  try {
    ...
    if (data.status_code === 0) {
      const apiMsg = data.message || data.status_message || JSON.stringify(data);
    ...
    if (!orderId || !shipmentId) {
      const msg = 'Shiprocket did not return order_id or shipment_id. Check pickup location and address.';
    ...
    const shipmentDetail = {
      pickup_location_added: data.pickup_location_added || res.data.pickup_location_added || 0,
      order_created: data.order_created ?? res.data.order_created ?? 1,
      awb_generated: awbData ? 1 : (data.awb_generated ?? res.data.awb_generated ?? 0),
      ...
      manifest_generated: data.manifest_generated ?? res.data.manifest_generated ?? 0,
      pickup_scheduled_date: data.pickup_scheduled_date || res.data.pickup_scheduled_date || null,
      pickup_booked_date: data.pickup_booked_date || res.data.pickup_booked_date || null,
      order_id: orderId,
      shipment_id: shipmentId,
      awb_code: awbData?.awb_code || data.awb_code || res.data.awb_code || '',
      courier_company_id: awbData?.courier_company_id || data.courier_company_id || res.data.courier_company_id || '',
      courier_name: awbData?.courier_name || data.courier_name || res.data.courier_name || '',
      assigned_date_time: awbData?.assigned_date_time || data.assigned_date_time || res.data.assigned_date_time || '',
      applied_weight: data.applied_weight ?? res.data.applied_weight ?? dimension.weight ?? 0.5,
      ...
      awb_assign_error: data.awb_assign_error || res.data.awb_assign_error || '',
    ...
    return {
      ...
      courierName: awbData?.courier_name || data.courier_name || res.data.courier_name || null,
    };
  } catch (err) {
    if (err.response?.data) {
      const d = err.response.data;
      const apiMsg = d.message || d.status_message || d.errors || (typeof d === 'string' ? d : JSON.stringify(d));
